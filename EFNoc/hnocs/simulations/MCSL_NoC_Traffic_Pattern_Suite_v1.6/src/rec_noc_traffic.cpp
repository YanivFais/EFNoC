...
int RecNOCTraffic::load_traffic(string trace_file) {
...
int	RecNOCTraffic::print_traffic() {
	...
	double messages_size = 0;
	double min_msg = 1e20;
	double max_msg = 0;
	int cnt = 0;
	for (vector<RecEdge>::iterator it = edge_list.begin(); it != edge_list.end() ; it++) {
		vector<double>& msgs_size = it->get_recorded_msg_size();
		for (vector<double>::iterator mIt = msgs_size.begin(); mIt != msgs_size.end() ; mIt++) {
			num_messages++;
			messages_size += *mIt;
			min_msg = min(min_msg, *mIt);
			max_msg = max(max_msg, *mIt);
	...
	cnt = 0;
	double avg = messages_size / num_messages;
	...
	for (vector<RecEdge>::iterator it = edge_list.begin(); it != edge_list.end() ; it++) {
		...
		for (vector<double>::iterator mIt = msgs_size.begin(); mIt != msgs_size.end() ; mIt++) {
			//cout << "" << *mIt << endl;
			double diff = *mIt - avg;
			sum_diff_sqr += diff*diff;
		}
	}
	cout << "Messages #" << num_messages << ",min=" << min_msg << ",max=" << max_msg << ",avg=" << avg << ",std=" << sqrt(sum_diff_sqr / num_messages) << endl;
